...
%type <st> stmt asgn print read if while block for clear_screen repeat place print_string read_string type_of
...
%token PRINT READ IF ELSE WHILE FOR PRINT_STRING READ_STRING THEN END_IF DO END_WHILE REPEAT UNTIL END_FOR FROM STEP TO CASE VALUE AND NOT OR CLEAR_SCREEN_TOKEN PLACE_TOKEN TYPE_OF END_CASE DEFAULT
...
%token <string> VARIABLE UNDEFINED CONSTANT BUILTIN KEYWORD
...
/*NEW in version 0.11*/
defaultValue: DEFAULT TWO_DOTS stmtlist
			{
...
asgn:   VARIABLE ASSIGNMENT exp 
	...
	| KEYWORD ASSIGNMENT exp
		{
			execerror("Semantic error in assignment: it is not allowed to assing value to keyword", $1);
		}
;
