...
def main():
    ...
    optimalCost = 937  # CHANGE THIS

    with open("Datasets/att48_d.txt", "r") as f:
        for line in f:
    ...
    plt.savefig('images/Runtime_dataset6_iterations1000_try5.png')
    plt.close()
    ...
    plt.xlabel('Iteration')
    plt.savefig('images/Solution_dataset6_iterations1000_try5.png')

    #Save the data to a csv file, whitout overwriting the previous data
    with open('HillClimbingData.csv', 'a') as f:
        #Save the dataset name, number of iterations, total time, average time, optimal solutions
        f.write("att48_d.txt, " + str(iterations) + ", " + str(time.time() - total_start_time) + ", " + str(
