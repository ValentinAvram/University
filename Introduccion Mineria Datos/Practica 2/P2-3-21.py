...
precisionArbol.append(scores.mean())
scores = cross_val_score(SVC(kernel='linear',C=1), datasetBC.data, datasetBC.target, cv=10, scoring='accuracy')
precisionSVC.append(scores.mean())

#Grafica
plt.plot(precisionArbol, label='Arbol de decision')
plt.plot(precisionSVC, label='SVC')
plt.legend()
plt.show()

print("Datasets: Iris, Diabetes, Ionosphere, Segment Challenge, Wine, Breast Cancer")
print("Precision Arbol de decision: ", precisionArbol)
